...
export async function GET(request: NextRequest) {
  ...
  const spreadsheetId = searchParams.get("spreadsheetId") || process.env.GOOGLE_SHEET_VIP_ID;
  const range = searchParams.get("range") || process.env.GOOGLE_SHEET_RANGE || "VIP Gen Tracker!A:Z";
  const headerRowIndex = Number(searchParams.get("headerRow") || "3");

  console.log("Search Params - requestId:", requestId, "spreadsheetId:", spreadsheetId, "range:", range, "headerRow:", headerRowIndex);
  ...
  try {
    ...
    const data = valueRows.slice(headerRowIdx + 1).map((row, rowIndex) => {
      ...
      headers.forEach((header: string, index: number) => {
        ...
        const formula = index < formulaRow.length ? formulaRow[index] || '' : '';
        ...
        if (formula && formula.startsWith('=')) {
          ...
          if (formula.includes('drive.google.com/file/d/')) {
            ...
            const fileIdMatch = formula.match(/drive\.google\.com\/file\/d\/([^\/]+)/);
          ...
          rowObject[header] = {
            ...
            imageUrl: fileId ? `https://lh3.googleusercontent.com/d/${fileId}` : null,
            driveUrl: fileId ? `https://drive.google.com/file/d/${fileId}/view` : null
          };
        } else {
    ...
    const matchingRows = data.filter((row) => {
      const cellValue = String(row[requestIdHeader]?.value || row[requestIdHeader] || '').trim();
    ...
    if (matchingRows.length === 0) {
      ...
      console.log(`No records found with Request ID: ${requestId}. Sample values:`, sampleData);
    ...
    console.log(`Found ${matchingRows.length} rows matching Request ID: ${requestId}`);
    return NextResponse.json({ 
      data: matchingRows,
